        ...
        <div class="edit-view">
          <div class="item flex-view">
            ...
            <div class="right-box avatar-box flex-view">
              <img v-if="tData.form && tData.form.avatar" :src="downloadImage(avatarRefresh)" class="avatar" />
              <img v-else :src="AvatarIcon" class="avatar" />
              <div class="change-tips flex-view">
                <a-upload name="file" accept="image/*" :multiple="false" :before-upload="beforeUpload" :key="fileKey">
                  <label>点击更换头像</label>
                </a-upload>
...
<script setup>
  ...
  import { BASE_URL, USER_AVATAR, USER_NAME } from '/@/store/constants';
  ...
  const beforeUpload = (file) => {
    // 改文件名
    fileKey = Math.random();
    const fileName = userStore.user_id + '.' + file.type.substring(6);
    const copyFile = new File([file], fileName);
    tData.form.avatar = fileName;
    tData.form.avatarFile = copyFile;
    return false;
  };

  const downloadImage = (param) => {
    // 下载文件
    return BASE_URL + '/api/file/download/avatar?name=' + userStore.user_avatar + '&' + param;
  };
  ...
  const submit = () => {
    ...
    if (tData.form.avatarFile) {
      userStore.user_avatar = undefined;
    ...
    updateUserInfoApi(formData)
      .then((res) => {
        ...
        if (tData.form.avatarFile) {
          ...
          userStore.user_avatar = localStorage.getItem(USER_AVATAR);
          avatarRefresh = Math.random();
        }
        if (tData.form.nickname) {
          localStorage.setItem(USER_NAME, tData.form.nickname);
          userStore.user_nickname = localStorage.getItem(USER_NAME);
        }
      })
      .catch((err) => {
        console.log(err.msg);
