    ...
    <div class="list-content">
      <div class="order-item-view" v-for="(item, index) in orderData" :key="index">
        <div class="header flex-view">
          ...
          <div class="right">
            <a-popconfirm v-if="item.status === '1'" title="确定取消订单？" ok-text="是" cancel-text="否" @confirm="handleCancel(item)">
              <a-button type="primary" size="small" style="margin-right: 24px; font-size: 18px">取消</a-button>
            </a-popconfirm>
            <a-button
              v-if="item.status === '1'"
              type="primary"
              size="small"
              style="margin-right: 24px; font-size: 18px; background-color: #87d068; border-color: #87d068"
        ...
        <div class="content flex-view">
          <div class="left-list">
            <div class="list-item flex-view" v-for="orderDetail in item.orderDetails">
              <div class="detail flex-between flex-view">
                <div class="flex-between flex-top flex-view">
                  <h2 class="name">{{ orderDetail.title }}</h2>
                  <span class="count">X {{ orderDetail.number }}</span>
                </div>
          ...
          <div class="right-info">
            ...
            <p class="name-phone">{{ item.receiverName }} {{ item.receiverPhone }} </p>
        ...
        <div class="bottom flex-view">
          <div class="left">
            <span class="text">共 {{ item.orderDetails.length }} 件商品</span>
          </div>
